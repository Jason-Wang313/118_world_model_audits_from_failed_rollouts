...
\usepackage{url}
\usepackage{booktabs}
\title{World-Model Audits From Failed Rollouts: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about robot world-model evaluation. The original research bet was: Use failed rollouts to localize which physical assumptions the world model lacks. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item WorldBench: Disambiguating Physics for Diagnostic Evaluation of World Models (2026)
\item LLM-Based Control for Simulated Physical Reasoning: Modular Evaluation in the NeurIPS Embodied Agent Interface Challenge (2026)
\item Fatigue life evaluation of root failure in welded joints based on displacement around the weld root (2025)
\item WorldGym: World Model as An Environment for Policy Evaluation (2025)
